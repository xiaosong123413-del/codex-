...
function createFakeClient() {
  ...
  const pagedChildren = new Map([
    ...
    ['GuuSw88cTi2b5Qkox9kce0bPnoc|page-2', {
      items: [
        nodesByToken.get('resource-child-2'),
        nodesByToken.get('LNqCw1uVhin6TzkKPLAcDYyVnof'),
      ],
      has_more: false,
    }],
    ['LNqCw1uVhin6TzkKPLAcDYyVnof|', {
      items: [nodesByToken.get('memory-entry-1')],
      has_more: false,
    }],
    ['resource-child-1|', {
  ...
  return {
    ...
    async get(url, options) {
      ...
      if (url === '/open-apis/docx/v1/documents/doc_resource_1/blocks') {
        assert.equal(options?.params?.page_size, 200);
        return {
          data: {
            items: [
              { block_id: 'blk_1', type: 'text', text: '段落 1' },
...
test('getJournalInputNodes 只返回 Memory 日期页下面的真实日记页', async () => {
  const service = new KnowledgeService(createFakeClient());

  const results = await service.scanRoots();
  const journalNodes = service.getJournalInputNodes(results);
  const resourceNodes = results.find((item) => item.root.key === 'resource')?.nodes ?? [];
  const memoryRoot = resourceNodes.find((node) => node.nodeToken === 'LNqCw1uVhin6TzkKPLAcDYyVnof');
  const memoryEntry = resourceNodes.find((node) => node.nodeToken === 'memory-entry-1');
  const genericResourceNode = resourceNodes.find((node) => node.nodeToken === 'resource-grandchild-1');

  assert.deepEqual(
    journalNodes.map((node) => node.nodeToken),
    ['memory-entry-1']
  );
  assert.equal(memoryRoot?.isJournalMemoryRoot, true);
  assert.equal(memoryRoot?.isJournalDatePage, false);
  assert.equal(memoryRoot?.isJournalEntry, false);
  assert.equal(memoryEntry?.isJournalEntry, true);
  assert.equal(memoryEntry?.journalPathDepth, 1);
  assert.equal(genericResourceNode?.isJournalEntry, false);
  assert.equal(genericResourceNode?.journalPathDepth, -1);
});

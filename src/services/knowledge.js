...
export class KnowledgeService {
  ...
  async scanRoot(root) {
    ...
    const descendants = await this.walkChildNodes(normalizedRootNode.spaceId, normalizedRootNode.nodeToken, root);
    return {
      root: { ...root },
  ...
  async walkChildNodes(spaceId, parentNodeToken, root) {
    const descendants = [];
  ...
  async listDocumentDescendants(documentId, blockId = '0', pageSize = 200) {
    ...
    do {
      const response = await this.client.post(
        `/open-apis/docx/v1/documents/${encodeURIComponent(documentId)}/blocks/${encodeURIComponent(blockId)}/descendant`,
        {
          page_size: pageSize,
